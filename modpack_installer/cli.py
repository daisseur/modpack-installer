...
from . import forge_install
from . import fabric_install
...
def main():
    parser = argparse.ArgumentParser(
        description="Install CurseForge modpacks on Linux systems",
        epilog="For more information, visit: https://github.com/daisseur/modpack-installer"
    )
    parser.add_argument('zipfile', help='Path to the modpack zip file')
    parser.add_argument('--manual', dest='forge_disable', action='store_true',
                       help='Use manual installation for modloader (opens GUI)')
    parser.add_argument(
        '--mcdir', dest='mcdir',
    ...
    parser.add_argument(
        ...
        help='Open manual download links in browser'
    )
    parser.add_argument(
        '-s', '--sandbox', action='store_true', dest='sandbox', default=None,
        help="Force 'sandbox' mode (for Flatpak etc.), places files alongside .minecraft dir"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--version', action='version', version='%(prog)s 2.3.5'
    )
